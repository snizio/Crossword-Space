"""Clue-to-clue baseline using MPNet sentence embeddings.

Encodes all training clues with MPNet, then for each test clue finds the most
similar training clues and returns their associated answers.
...
from crossword_space.model import CrosswordDataset, mean_pooling
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(dataset_loader, desc="Encoding clue bank"):
            ...
            sentence_embeddings = mean_pooling(model_output.last_hidden_state, clue_attention_mask)
    ...
    length_faiss_raw = defaultdict(lambda: ([], []))
    for clue in clues:
    ...
    for data_source in ["crossword", "dict", "onli", "neo"]:
        test_data = CrosswordDataset("datasets/dict_test.csv", tokenizer, data_source=[data_source])
        ...
        with torch.no_grad():
            ...
            for batch in test_loader:
                ...
                def_embeddings = mean_pooling(model_output.last_hidden_state, def_attention_mask)
        ...
        df_preds.to_csv(f"results/baseline_c2c/predictions-{data_source}.csv", index=False)
        ...
        with open(f"results/baseline_c2c/results-{data_source}.json", "w") as f:
