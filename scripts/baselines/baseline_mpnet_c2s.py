"""Clue-to-solution baseline using MPNet sentence embeddings.

Encodes all candidate solutions with MPNet, then for each test clue encodes it
and finds the most similar solution embeddings via FAISS.
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(dataset_loader, desc="Encoding items"):
            ...
            sentence_embeddings = mean_pooling(model_output.last_hidden_state, ans_attention_mask)
    ...
    for data_source in ["crossword", "dict", "onli", "neo"]:
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                sentence_embeddings = mean_pooling(model_output.last_hidden_state, def_attention_mask)
        ...
        df_preds.to_csv(f"results/baseline_c2s/predictions-{data_source}.csv", index=False)
        ...
        with open(f"results/baseline_c2s/results-{data_source}.json", "w") as f:
