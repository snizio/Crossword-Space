...
from crossword_space.model import (CrosswordDataset, DualEncoderADE,
...
if tied_conf == "ade": # we load two encoders
    ...
    dual_encoder = DualEncoderADE.from_encoders_pretrained(model_name, projection_dim)

    if parallel == "parallel":
        dual_encoder.encoder_def.to(device_def)
        dual_encoder.encoder_ans.to(device_ans)
        dual_encoder.layer_norm.to(device_ans)
        dual_encoder.projection.to(device_ans)
    else:
        dual_encoder.to(device_def)
    ...
    for step, batch in enumerate(train_loader):
        ...
        if (step + 1) % eval_steps == 0:
            ...
            if avg_val_loss < best_val_loss:
                ...
                checkpoint_path = os.path.join(checkpoint_folder_path, f"checkpoint-{global_step}")
                dual_encoder.save_pretrained(checkpoint_path)
...
# Load the best model
if tied_conf == "ade":
    dual_encoder = AutoModel.from_pretrained(checkpoint_path)
    if parallel == "parallel":
        dual_encoder.encoder_def.to(device_def)
        dual_encoder.encoder_ans.to(device_ans)
        dual_encoder.layer_norm.to(device_ans)
